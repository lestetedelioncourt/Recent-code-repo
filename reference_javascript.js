...
$(document).ready(function(){
    ...
    $(function() {
        $('#color-block1').on('colorchange', function(e) {
            xcolor = $(this).wheelColorPicker('value');
            var alpha1 = $(this).wheelColorPicker('color').a;
            $('#xEdgeColor').css('background-color', xcolor);
            $('#xEdgeColorText').text(xcolor);
            $('#xEdgeColorAlpha').text(Math.round(alpha1 * 100) + '%');
        });
    });

    $(function() {
        $('#color-block2').on('colorchange', function(e) {
            ycolor = $(this).wheelColorPicker('value');
            var alpha2 = $(this).wheelColorPicker('color').a;
            $('#yEdgeColor').css('background-color', ycolor);
            $('#yEdgeColorText').text(ycolor);
            $('#yEdgeColorAlpha').text(Math.round(alpha2 * 100) + '%');
        });
    });

    $(function() {
        $('#color-block3').on('colorchange', function(e) {
            xycolor = $(this).wheelColorPicker('value');
            var alpha3 = $(this).wheelColorPicker('color').a;
            $('#xyEdgeColor').css('background-color', xycolor);
            $('#xyEdgeColorText').text(xycolor);
            $('#xyEdgeColorAlpha').text(Math.round(alpha3 * 100) + '%');
        });
    });

    $(function() {
        $('#color-block4').on('colorchange', function(e) {
            var yxcolor = $(this).wheelColorPicker('value');
            var alpha4 = $(this).wheelColorPicker('color').a;
            $('#yxEdgeColor').css('background-color', yxcolor);
            $('#yxEdgeColorText').text(yxcolor);
            $('#yxEdgeColorAlpha').text(Math.round(alpha4 * 100) + '%');
        });
    });
	...
	$('#show-popup-btn').click(function() {
		...
		$('.jQWCP-slider-wrapper').css({'width': '29.75px'});
	})
    ...
    $('#edgeBtn').click(function() {
        if (preblur != '') {
            $('#newForm').append('<input type="hidden" value="' + preblur + '" name="preblur"/>');
        }

        if (bandpassmax != '') {
            $('#newForm').append('<input type="hidden" value="' + bandpassmax + '" name="bandpassmax"/>');
        }

        if (bandpassmin != '') {
            $('#newForm').append('<input type="hidden" value="' + bandpassmin + '" name="bandpassmin"/>');
        }

        if (edgethreshold != '') {
            $('#newForm').append('<input type="hidden" value="' + edgethreshold + '" name="edgethreshold"/>');
        }

        if (xcolor != '') {
            $('#newForm').append('<input type="hidden" value="' + xcolor + '" name="xcolor"/>');
        }

        if (ycolor != '') {
            $('#newForm').append('<input type="hidden" value="' + ycolor + '" name="ycolor"/>');
        }

        if (xycolor != '') {
            $('#newForm').append('<input type="hidden" value="' + xycolor + '" name="xycolor"/>');
        }

        if (yxcolor != '') {
            $('#newForm').append('<input type="hidden" value="' + yxcolor + '" name="yxcolor"/>');
        }
    });

    $('#houghBtn').click(function() {
        if (minColony != '') {
            $('#newForm').append('<input type="hidden" value="' + minColony + '" name="minColony"/>');
        }

        if (maxColony != '') {
            $('#newForm').append('<input type="hidden" value="' + maxColony + '" name="maxColony"/>');
        }

        if (houghThickness != '') {
            $('#newForm').append('<input type="hidden" value="' + houghThickness + '" name="houghThickness"/>');
        }

        if (houghThreshold != '') {
            $('#newForm').append('<input type="hidden" value="' + houghThreshold + '" name="houghThreshold"/>');
        }
    });

    $('#autoHoughBtn').click(function() {
        if (xAuto != '') {
            $('#newForm').append('<input type="hidden" value="' + yAuto + '" name="yAuto"/>');
        }
        if (yAuto != '') {
            $('#newForm').append('<input type="hidden" value="' + xAuto + '" name="xAuto"/>');
        }
    });
    ...
    $( "#preblur" ).slider({
		...
		slide: function( event, ui ) {
			$('#preblurtext').html("PreBlur : " + ui.value);
			preblur = ui.value;
		}
    });
    $( "#bandpassmax" ).slider({
        ...
        max: 256,
		slide: function( event, ui ) {
			$('#bandpassmaxtext').html("Bandpass Maximum : " + ui.value);
			bandpassmax = ui.value;
		}
    });
    $( "#bandpassmin" ).slider({
        min: 0,
        max: 256,
		slide: function( event, ui ) {
			$('#bandpassmintext').html("Bandpass Minimum : " + ui.value);
			bandpassmin = ui.value;
		}
    });
    $( "#edgethreshold" ).slider({
        min: 0,
        max: 100,
		slide: function( event, ui ) {
			$('#edgethresholdtext').html("Edge Threshold : " + ui.value);
			edgethreshold = ui.value;
		}
    });
    $( "#minColony" ).slider({
        min: 1,
        max: 50,
		slide: function( event, ui ) {
			$('#minColonyText').html("Minimum colony size : " + ui.value);
			minColony = ui.value;
		}
    });
    $( "#maxColony" ).slider({
        ...
        max: 60,
		slide: function( event, ui ) {
			$('#maxColonyText').html("Maximum colony size : " + ui.value);
			maxColony = ui.value;
		}

    });
    $( "#houghThickness" ).slider({
        min: 1,
        max: 10,
		slide: function( event, ui ) {
			$('#houghThicknessText').html("Hough Thickness : " + ui.value);
			houghThickness = ui.value;
		}

    });
    $( "#houghThreshold" ).slider({
        min: 1,
        max: 100,
		slide: function( event, ui ) {
			$('#houghThresholdText').html("Hough Threshold : " + ui.value);
			houghThreshold = ui.value;
		}
    });
    ...
    $( "#xAuto" ).slider({
        ...
        step: 0.01,
        slide: function( event, ui ) {
            $('#xAutoText').html("X Auto modifier: " + ui.value);
            xAuto = ui.value;
        }
    });
    ...
    $( "#yAuto" ).slider({
        ...
        step: 0.01,
        slide: function( event, ui ) {
            $('#yAutoText').html("Y Auto modifier: " + ui.value);
            yAuto = ui.value;
        }
    });
    ...
    $('img').bind('mousedown mouseup', function(event) {
    	if (event.type == 'mousedown') {
    		...
    		if ($('input[type="radio"]:checked').val() == "manualEllipse") {
    		    /*startY -= height * 0.2071;
    		    startX -= width * 0.2071;
                height *= 1.4142;
                width *= 1.4142*/
    		...
    		else if ($('input[type="radio"]:checked').val() == "manualRectangle") {
    			radius = 0;
    		}
    		else {
    		    return;
